...
CREATE TABLE file_cleanup_runs (
  ...
  run_type varchar(20) NOT NULL CHECK (run_type IN ('scan','cleanup')), -- Jenis pekerjaan worker.
  source_scan_run_id bigint REFERENCES file_cleanup_runs(id), -- Pemeriksaan sumber untuk pekerjaan pembersihan.
  status varchar(20) NOT NULL DEFAULT 'queued' CHECK (status IN ('queued','running','completed','partial','failed','cancelled')), -- Status antrean.
  requested_by_user_id bigint NOT NULL REFERENCES users(id), -- Superadmin pemohon.
  ...
  last_error_code varchar(80), -- Kode aman tanpa path atau detail internal.
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT uq_file_cleanup_runs_org_id UNIQUE (organization_id,id),
  CONSTRAINT fk_file_cleanup_source_scan FOREIGN KEY (organization_id,source_scan_run_id) REFERENCES file_cleanup_runs(organization_id,id),
  CONSTRAINT ck_file_cleanup_source CHECK ((run_type='scan' AND source_scan_run_id IS NULL) OR (run_type='cleanup' AND source_scan_run_id IS NOT NULL))
);
COMMENT ON TABLE file_cleanup_runs IS 'Antrean dan histori pemeriksaan atau pembersihan file per organisasi.';
CREATE INDEX ix_file_cleanup_runs_queue ON file_cleanup_runs(next_attempt_at,id) WHERE status='queued';
CREATE UNIQUE INDEX uq_file_cleanup_runs_active_org ON file_cleanup_runs(organization_id) WHERE status IN ('queued','running');
CREATE INDEX ix_file_cleanup_runs_org_time ON file_cleanup_runs(organization_id,created_at DESC,id DESC);
