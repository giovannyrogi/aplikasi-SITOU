...
CREATE TEMP TABLE _034_dependent_contact_source ON COMMIT DROP AS
SELECT
  dependent.id,
...
CREATE TEMP TABLE _034_emergency_contact_source ON COMMIT DROP AS
SELECT
  contact.id,
...
CREATE TEMP TABLE _034_existing_matches ON COMMIT DROP AS
SELECT
  dependent.id AS dependent_id,
  EXISTS (
    SELECT 1
    FROM _034_emergency_contact_source contact
...
FROM _034_dependent_contact_source dependent
WHERE EXISTS (
  SELECT 1
  FROM _034_emergency_contact_source contact
  WHERE contact.organization_id = dependent.organization_id
...
CREATE TEMP TABLE _034_new_candidates ON COMMIT DROP AS
SELECT dependent.*
FROM _034_dependent_contact_source dependent
WHERE NOT EXISTS (
  SELECT 1
...
BEGIN
  SELECT count(*) INTO conflict_count FROM _034_conflicts;
  IF conflict_count > 0 THEN
    ...
    FROM (SELECT * FROM _034_conflicts ORDER BY organization_id, employee_id LIMIT 10) sample;
...
CREATE TEMP TABLE _034_created_contacts ON COMMIT DROP AS
...
CREATE TEMP TABLE _034_primary_assignments ON COMMIT DROP AS
WITH eligible AS (
...
DECLARE
  ...
  conflict_count integer;
BEGIN
  SELECT count(*) INTO source_count FROM _034_dependent_contact_source;
  SELECT count(*) INTO matched_count FROM _034_existing_matches;
  SELECT count(*) INTO ignored_count FROM _034_existing_matches WHERE NOT exact_match;
  SELECT count(*) INTO created_count FROM _034_created_contacts;
  SELECT count(*) INTO primary_count FROM _034_primary_assignments;
  SELECT count(*) INTO conflict_count FROM _034_conflicts;

  RAISE NOTICE
    'dependent contact migration 034: source=%, matched=%, created=%, ignored_by_emergency_contact=%, primary_assigned=%, conflict=%',
    source_count,
    matched_count,
    created_count,
    ignored_count,
