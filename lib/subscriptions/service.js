...
export async function listOrganizationSubscriptions(organizationId, database = pool) {
  ...
  const result = await database.query(
    ...
    os.notes,os.created_by_user_id,identity.display_name AS created_by_name,os.created_at,os.updated_at
    FROM organization_subscriptions os JOIN organizations o ON o.id=os.organization_id
    LEFT JOIN v_user_identity identity ON identity.user_id=os.created_by_user_id WHERE os.organization_id=$1
