...
const organizationUnitLocationSchema = z.object({
  locationId: z.coerce.number().int().positive("Lokasi tidak valid."),
  activeFrom: dateSchema,
});

const organizationUnitLocationsSchema = z
  .array(organizationUnitLocationSchema)
  .max(100)
  .default([])
  .superRefine((locations, context) => {
    const seen = new Set();
    locations.forEach((location, index) => {
      const key = String(location.locationId);
      if (seen.has(key))
        context.addIssue({
          code: "custom",
          path: [index, "locationId"],
          message: "Lokasi yang sama tidak boleh dipilih lebih dari sekali.",
        });
      seen.add(key);
    });
  });
...
const organizationUnitBaseSchema = z.object({
  ...
  locations: organizationUnitLocationsSchema,
  locationChangeReason: nullableText(2000),
  locationIds: z
    .unknown()
    .refine(() => false, "Gunakan lokasi beserta tanggal mulai berlakunya.")
    .optional(),
  isActive: z.boolean().default(true),
});
