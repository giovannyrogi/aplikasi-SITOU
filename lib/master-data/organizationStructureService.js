...
/** Memastikan seluruh lokasi pilihan aktif dan berada pada organisasi yang sama. */
const validateUnitLocations = async (client, organizationId, locationIds) => {
  const uniqueIds = [...new Set(locationIds.map(String))];
  if (!uniqueIds.length) return [];

  const result = await client.query(
    `SELECT id::text FROM locations WHERE organization_id=$1 AND id=ANY($2::bigint[])
      AND is_active=true AND operational_from<=current_date
  ...
  if (result.rowCount !== uniqueIds.length)
    throw new ServiceError(
      "LOCATION_INVALID",
      "Semua lokasi unit harus aktif dan berasal dari organisasi yang sama.",
      400,
    );
  return uniqueIds;
};
...
/** Menjaga histori pemetaan lokasi dengan menutup relasi lama dan menambah relasi baru. */
const synchronizeUnitLocations = async (client, organizationId, unitId, locationIds) => {
  const current = await client.query(
    `SELECT location_id::text,active_from::text FROM organization_unit_locations
  ...
  const desired = new Set(locationIds.map(String));
  const existing = new Set(current.rows.map((row) => row.location_id));

  for (const row of current.rows) {
    if (desired.has(row.location_id)) continue;
    await client.query(
      `DELETE FROM organization_unit_locations
        WHERE organization_id=$1 AND organization_unit_id=$2 AND location_id=$3
        AND active_from=$4 AND active_from=current_date`,
  ...
  for (const locationId of desired) {
    ...
    await client.query(
      ...
      [organizationId, unitId, locationId],
    );
  }
};
...
export async function createOrganizationUnit(input, actor, requestId) {
  try {
    return await withTransaction(async (client) => {
      ...
      const locationIds = await validateUnitLocations(
        client,
      ...
      await synchronizeUnitLocations(client, input.organizationId, id, locationIds);
