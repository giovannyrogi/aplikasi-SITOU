      ...
      : row.lifecycle_status === "draft"
        ...
        : (row.reference_labels || []).length
          ...
          : row.lifecycle_status === "active"
            ? "Metadata aktif tanpa referensi"
            : "Dilepas dari data dan menunggu purge",
  };
};
...
export async function getStorageMaintenanceRun(
  ...
  const itemSelect = `SELECT item.id::text,item.stored_file_id::text,
    ...
    COALESCE(file.original_name,'File fisik tanpa metadata') AS original_name,
