...
/** Menegakkan kewajiban surat kembali di service agar tidak bergantung pada satu entry point API. */
function ensureRequiredDisciplineLetter(input) {
  const writtenActive = input.actionType !== "oral_warning" && input.status === "active";
  if (!writtenActive) return;
  if (!input.letterNo)
...
export async function createDisciplinaryAction(caseId, input, actor, requestId) {
  return withTransaction(async (client) => {
    ensureRequiredDisciplineLetter(input);
...
export async function getEmployeeDisciplineHistory(employeeId, organizationId, actor) {
  const includeDraftActions = canViewDraftDisciplinaryActions(actor);
