...
export const disciplinaryActionCreateSchema = z
  ...
  .superRefine((value, context) => {
    ...
    // Draft boleh belum lengkap; seluruh surat tertulis wajib lengkap saat diterbitkan.
    const requiresWrittenDocument = value.status === "active" && written;
    if (requiresWrittenDocument && !value.letterNo)
      context.addIssue({ code: "custom", path: ["letterNo"], message: "Nomor surat wajib diisi." });
    if (requiresWrittenDocument && !value.documentFileId)
      context.addIssue({
        code: "custom",
        path: ["documentFileId"],
        message: "Dokumen surat wajib diunggah sebelum tindakan disimpan.",
      });
...
export const disciplinaryActionRevokeSchema = z.object({
  organizationId: id("Organisasi wajib dipilih."),
  reason: z
    .string()
    .trim()
    .min(10, "Alasan pencabutan minimal 10 karakter.")
    .max(3000, "Alasan pencabutan maksimal 3000 karakter."),
});
