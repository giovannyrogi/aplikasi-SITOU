...
export async function getSelfProfile(userId, database = pool) {
  ...
  if (!result.rows[0]) throw new ServiceError("PROFILE_NOT_FOUND", "Profil akun tidak ditemukan.", 404);
...
export async function updateSelfProfile(input, actor, requestId) {
  return withTransaction(async (client) => {
    ...
    if (before.identity_source === "employee") {
      if (!before.employee_id || String(actor.organization_id) !== String(before.employee_organization_id || actor.organization_id))
        throw new ServiceError("PROFILE_FORBIDDEN", "Profil pegawai tidak berada pada organisasi akun.", 403);
      await client.query(
      ...
      await client.query(
        ...
        [actor.organization_id, before.employee_id, input.personalEmail || null, input.whatsapp || null],
      );
      ...
      await client.query(
        `INSERT INTO platform_user_profiles(user_id,full_name,email,whatsapp)
         ...
         ON CONFLICT (user_id) DO UPDATE SET full_name=excluded.full_name,email=excluded.email,
           whatsapp=excluded.whatsapp,updated_at=now()`,
        [actor.id, input.fullName || before.platform_full_name, input.email || null, input.whatsapp || null],
      );
    } else {
...
export async function changeSelfPassword(input, actor, requestId) {
  return withTransaction(async (client) => {
    const result = await client.query("SELECT password_hash FROM users WHERE id=$1 FOR UPDATE", [actor.id]);
    const passwordMatches = await bcrypt.compare(input.currentPassword, result.rows[0]?.password_hash || "");
    if (!passwordMatches)
