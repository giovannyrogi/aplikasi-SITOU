...
import { getReportDashboardMetrics } from "@/lib/reports/service";
...
/** Mengambil snapshot komposisi tenaga kerja aktif untuk scope organisasi dan lokasi actor. */
async function queryEmployeeSummary({ organizationId = null, scopedLocationIds = null } = {}) {
  return pool.query(
    `WITH scoped AS (
       SELECT employee.*
       ...
       WHERE employee.deleted_at IS NULL
         ...
         AND ($1::bigint IS NULL OR employee.organization_id=$1::bigint)
       ...
       SELECT scoped.*,
         current_contract.employment_type_name
       FROM scoped
       LEFT JOIN LATERAL (
         SELECT employment_type.name AS employment_type_name
         ...
         WHERE contract.organization_id=scoped.organization_id
           AND contract.employee_id=scoped.id
       ...
       SELECT 'gender'::text AS dimension,COALESCE(gender,'undisclosed')::text AS label FROM enriched
       UNION ALL
       SELECT 'status',employment_status::text FROM enriched
       UNION ALL
       SELECT 'tenure',CASE
         WHEN joined_date>current_date-interval '1 year' THEN 'under_1'
         WHEN joined_date>current_date-interval '3 years' THEN '1_to_3'
         WHEN joined_date>=current_date-interval '5 years' THEN '3_to_5'
         ELSE 'over_5'
       END
       FROM enriched WHERE joined_date IS NOT NULL
       UNION ALL
       SELECT 'employment_type',COALESCE(NULLIF(trim(employment_type_name),''),'Belum ditentukan')
       FROM enriched
     )
     SELECT dimension,label,count(*)::int AS value
     FROM dimensions
...
async function getOrganizationDashboard({ organizationId, startDate, endDate, scopedLocationIds }) {
  ...
  const [
    ...
    pool.query(
      `WITH scoped AS (SELECT employee.* FROM employees employee WHERE ${employeeScope})
         SELECT
           ...
           count(*) FILTER (WHERE employment_status IN ('active','probation') AND NOT EXISTS (
           ...
           count(*) FILTER (WHERE ${incompleteEmployeeSql("scoped")})::int AS incomplete_profiles,
           (SELECT count(*)::int FROM disciplinary_actions action JOIN scoped s ON s.id=action.employee_id
             WHERE action.organization_id=$1 AND action.status='active'
               AND (action.effective_until IS NULL OR action.effective_until>=current_date)) AS active_actions,
           (SELECT count(DISTINCT request.employee_id)::int FROM leave_requests request
             JOIN scoped s ON s.id=request.employee_id
             WHERE request.organization_id=$1 AND request.status='approved'
               AND current_date BETWEEN request.start_at::date AND request.end_at::date) AS current_leave
         FROM scoped`,
      params,
    ),
    pool.query(
      `SELECT unit.name AS label,count(DISTINCT employee.id)::int AS value
         FROM employees employee
         ...
         WHERE ${employeeScope} AND employee.employment_status IN ('active','probation')
           AND assignment.assignment_type='primary' AND assignment.effective_from<=current_date
           AND (assignment.effective_until IS NULL OR assignment.effective_until>=current_date)
         GROUP BY unit.id,unit.name ORDER BY value DESC,unit.name`,
      params,
    ),
    pool.query(
      `WITH months AS (
           SELECT month,
             GREATEST(month,$2::date) AS period_start,
             LEAST((month+'1 month'::interval-'1 day'::interval)::date,$3::date) AS snapshot_date
         ...
         SELECT to_char(months.month,'YYYY-MM') AS period,
           count(scoped.id) FILTER (WHERE scoped.joined_date BETWEEN months.period_start AND months.snapshot_date)::int AS new_hires,
           count(scoped.id) FILTER (WHERE scoped.termination_date BETWEEN months.period_start AND months.snapshot_date)::int AS departures
         FROM months LEFT JOIN scoped ON true
         GROUP BY months.month,months.period_start,months.snapshot_date ORDER BY months.month`,
      params,
    ),
    pool.query(
      `SELECT location.name AS label,count(DISTINCT employee.id)::int AS value
         ...
         WHERE ${employeeScope} AND employee.employment_status IN ('active','probation')
           AND assignment.assignment_type='primary' AND assignment.effective_from<=current_date
           AND (assignment.effective_until IS NULL OR assignment.effective_until>=current_date)
         GROUP BY location.id,location.name ORDER BY value DESC,location.name`,
      params,
    ),
    pool.query(
      `WITH months AS (SELECT generate_series(date_trunc('month',$2::date),date_trunc('month',$3::date),'1 month')::date AS month),
         ...
         FROM months LEFT JOIN employment_contracts contract
           ...
           AND contract.end_date BETWEEN $2::date AND $3::date
    ...
    pool.query(
      `WITH scoped AS (SELECT employee.* FROM employees employee WHERE ${employeeScope}), scored AS (
           ...
           FROM scoped WHERE scoped.employment_status IN ('active','probation'))
    ...
    pool.query(
      `WITH scoped AS (SELECT employee.* FROM employees employee WHERE ${employeeScope})
         SELECT case_data.id::text AS case_id,scoped.id::text AS id,scoped.full_name AS title,
           case_data.severity,to_char(case_data.incident_date,'DD Mon YYYY') AS incident_date
         FROM discipline_cases case_data JOIN scoped ON scoped.id=case_data.employee_id
         WHERE case_data.organization_id=$1 AND EXISTS (
         ...
         ORDER BY case_data.incident_date DESC,case_data.id DESC LIMIT 5`,
      params,
    ),
    pool.query(
      `WITH scoped AS (SELECT employee.* FROM employees employee WHERE ${employeeScope})
         SELECT * FROM (
           ...
           FROM scoped WHERE scoped.employment_status IN ('active','probation') AND NOT EXISTS (
           ...
           WHERE action.status='active'
             AND (action.effective_until IS NULL OR action.effective_until>=current_date)
         ) item ORDER BY priority,due_at NULLS LAST,type,id LIMIT 5`,
      params,
    ),
    pool.query(
    ...
    queryEmployeeSummary({ organizationId, scopedLocationIds }),
    pool.query(BIRTHDAY_SUMMARY_SQL, [organizationId, endDate, scopedLocationIds]),
  ]);
  ...
  return {
    ...
    metrics: [
      ...
      },
      {
        ...
        value: asNumber(m.current_leave),
        tone: "success",
        icon: "solar:calendar-mark-bold-duotone",
        href: `/leave-requests?organizationId=${organizationId}`,
      },
    ],
    charts: {
      growth: {
        ...
        series: [
          {
            name: "Pegawai baru",
            data: growth.rows.map((r) => asNumber(r.new_hires)),
          },
          {
            name: "Pegawai keluar",
            data: growth.rows.map((r) => asNumber(r.departures)),
          },
        ],
      },
      locations: {
      ...
      units: {
        categories: units.rows.map((r) => r.label),
        series: [{ name: "Pegawai", data: units.rows.map((r) => asNumber(r.value)) }],
      },
    },
    employeeSummary: buildEmployeeSummary(employeeSummary.rows),
    birthdaySummary: buildBirthdaySummary(birthdays.rows, endDate),
    recentDiscipline: discipline.rows.map((row) => ({
      id: row.id,
      caseId: row.case_id,
      type: "discipline",
      title: row.title,
      description: `Pelanggaran ${formatDisciplineSeverity(row.severity).toLowerCase()} - ${row.incident_date}`,
    })),
...
async function getPlatformDashboard({ startDate, endDate }) {
  const [
    ...
    pool.query(`SELECT organization.id::text,organization.name AS title,
      ...
      ORDER BY priority,due_at NULLS LAST,organization.id LIMIT 5`),
    pool.query(
      `SELECT audit.id::text,audit.action,audit.entity_type,audit.occurred_at,identity.display_name AS actor_name
      FROM audit_logs audit LEFT JOIN v_user_identity identity ON identity.user_id=audit.actor_user_id
      ORDER BY audit.occurred_at DESC,audit.id DESC LIMIT 5`,
    ),
    queryEmployeeSummary(),
  ]);
  const m = metrics.rows[0];
  return {
    scope: "platform",
...
export async function getDashboardSummary({ actor, requestedOrganizationId }) {
  ...
  const organizationTimezone = organizationId
    ? await pool.query("SELECT timezone FROM organizations WHERE id=$1", [organizationId])
    : null;
  const range = getDashboardTrendRange(organizationTimezone?.rows[0]?.timezone || "Asia/Makassar");
  // Versi kebijakan menghindari cache lama, termasuk pada proses aplikasi yang berbeda.
  const policyVersion = organizationId
    ? (
        await pool.query(
          "SELECT version FROM organization_retirement_policies WHERE organization_id=$1",
          [organizationId],
        )
      ).rows[0]?.version || 0
    : 0;
  const cacheKey = [
    ...
    range.startDate,
    range.endDate,
    locationScope === null ? "all" : `selected:${locationScope.join(",")}`,
  ].join(":");
  const cached = readCache(cacheKey);
  if (cached) return cached;

  const dashboard = organizationId
    ? await getOrganizationDashboard({ organizationId, scopedLocationIds: locationScope, ...range })
    : await getPlatformDashboard(range);
  if (organizationId) {
    const reportMetrics = await getReportDashboardMetrics(actor, String(organizationId));
    dashboard.metrics = dashboard.metrics.map((metric) =>
      metric.key === "expiringContracts"
        ? { ...reportMetrics[0], label: "Kontrak berakhir dalam 30 hari" }
        : metric,
    );
    dashboard.retirementSummary = {
      asOf: reportMetrics[1].asOf,
      upcoming: reportMetrics[1],
      overdue: reportMetrics[2],
    };
  }
