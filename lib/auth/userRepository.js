...
const employeeAccessJoins = (organizationIdExpression) => `
  ...
  LEFT JOIN LATERAL (
    SELECT assignment.id,location.is_active AS location_is_active,
      ...
      organization_unit.is_active AS unit_is_active,position.name AS position_name
    ...
    JOIN organization_units organization_unit ON organization_unit.organization_id=assignment.organization_id AND organization_unit.id=assignment.organization_unit_id
    ...
    WHERE assignment.organization_id=${organizationIdExpression}
      AND assignment.employee_id=employee_profile.id AND assignment.assignment_type='primary'
      AND assignment.effective_from<=(now() AT TIME ZONE o.timezone)::date
      AND (assignment.effective_until IS NULL OR assignment.effective_until>=(now() AT TIME ZONE o.timezone)::date)
...
const employeeAccessColumns = `employee_profile.id AS employee_id,
  ...
  employee_assignment.position_name`;

export async function findUserForLogin(username) {
  const result = await pool.query(
    `SELECT u.id,u.username,u.password_hash,u.is_active,u.credential_version,
    identity.display_name,identity.preferred_name,identity.identity_source,identity.contact_email,identity.whatsapp,
    ...
    FROM users u JOIN v_user_identity identity ON identity.user_id=u.id
    LEFT JOIN LATERAL (
      SELECT uor.organization_id,uor.id AS role_assignment_id,r.code AS role_code,r.scope AS role_scope,uor.location_scope_mode
    ...
    ${subscriptionLateral} ${employeeAccessJoins("active_role.organization_id")}
...
export async function findActiveSessionUser({
  ...
  const result = await pool.query(
    `SELECT u.id,u.username,u.is_active,u.credential_version,identity.display_name,
    identity.preferred_name,identity.identity_source,identity.contact_email,identity.whatsapp,
    r.code AS role_code,r.scope AS role_scope,uor.organization_id,uor.id AS role_assignment_id,uor.location_scope_mode,
    ...
    subscription.effective_status AS organization_subscription_status,${employeeAccessColumns}
    FROM users u JOIN v_user_identity identity ON identity.user_id=u.id
    JOIN user_organization_roles uor ON uor.user_id=u.id JOIN roles r ON r.id=uor.role_id
    LEFT JOIN organizations o ON o.id=uor.organization_id ${subscriptionLateral}
    ${employeeAccessJoins("uor.organization_id")}
    WHERE u.id=$1 AND u.is_active=true AND u.credential_version=$4 AND r.code=$2
      AND uor.organization_id IS NOT DISTINCT FROM $3::bigint
      AND uor.active_from<=now() AND (uor.active_until IS NULL OR uor.active_until>now())
      AND (r.scope='platform' OR (
        o.is_active=true AND subscription.effective_status IN ('active','grace')
        AND (r.code<>'hrd' OR uor.location_scope_mode='all' OR EXISTS (
          SELECT 1 FROM user_location_scopes uls JOIN locations l ON l.organization_id=uls.organization_id AND l.id=uls.location_id
          WHERE uls.user_organization_role_id=uor.id AND l.is_active
            AND l.operational_from<=(now() AT TIME ZONE o.timezone)::date
            AND (l.operational_until IS NULL OR l.operational_until>=(now() AT TIME ZONE o.timezone)::date)))
        AND (r.code<>'employee' OR (employee_profile.id IS NOT NULL AND employee_profile.deleted_at IS NULL
          AND employee_profile.employment_status IN ('active','probation')
          AND employee_assignment.id IS NOT NULL AND employee_assignment.location_is_active=true
          AND employee_assignment.location_is_operational=true AND employee_assignment.unit_is_active=true))
      )) LIMIT 1`,
    [userId, roleCode, organizationId, credentialVersion],
  );
  return result.rows[0] || null;
}
