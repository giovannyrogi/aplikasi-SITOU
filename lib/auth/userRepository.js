...
export async function findActiveSessionUser({ userId, roleCode, organizationId, credentialVersion }) {
  const result = await pool.query(
    `SELECT u.id,u.username,u.is_active,u.credential_version,identity.display_name,
    identity.identity_source,identity.contact_email,identity.whatsapp,
...
export async function recordSuccessfulLogin({ user, ipAddress, userAgent, requestId }) {
  ...
  try {
    ...
    await client.query("UPDATE users SET last_login_at=now(),last_login_ip=$2::inet WHERE id=$1", [user.id,ipAddress]);
    await client.query(
      ...
      [user.organization_id,user.id,JSON.stringify({ role_code: user.role_code }),ipAddress,userAgent,requestId],
    );
