...
const getSigningKey = () =>
  crypto.subtle.importKey("raw", encoder.encode(getSessionSecret()), { name: "HMAC", hash: "SHA-256" }, false, ["sign", "verify"]);

export const getSessionTtlSeconds = () => {
  const configuredMinutes = Number(process.env.AUTH_SESSION_TTL_MINUTES);
  const minutes = Number.isFinite(configuredMinutes) && configuredMinutes > 0 ? configuredMinutes : DEFAULT_SESSION_TTL_MINUTES;
  return Math.floor(minutes * 60);
};

export async function createSessionToken(session) {
  if (!Number.isInteger(Number(session.credentialVersion)) || Number(session.credentialVersion) < 1)
    throw new Error("Versi kredensial session tidak valid.");
...
export async function verifySessionToken(token) {
  ...
  try {
    const key = await getSigningKey();
    const validSignature = await crypto.subtle.verify("HMAC", key, decodeBase64Url(encodedSignature), encoder.encode(encodedPayload));
    if (!validSignature) return null;
    const payload = JSON.parse(new TextDecoder().decode(decodeBase64Url(encodedPayload)));
    if (!payload?.userId || !payload?.roleCode || !payload?.expiresAt || !Number.isInteger(Number(payload.credentialVersion))) return null;
