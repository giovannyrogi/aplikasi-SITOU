...
/** Menghasilkan alasan penolakan login karyawan berdasarkan profil dan penempatan aktifnya. */
export function getEmployeeAccessFailure(user) {
  ...
  if (!user.employee_id || user.employee_deleted_at) {
    return {
      ...
      message:
        "Profil karyawan Anda tidak tersedia atau telah dinonaktifkan. Hubungi Admin organisasi Anda.",
    };
  }
  ...
  if (!EMPLOYEE_ALLOWED_STATUSES.has(user.employment_status)) {
    const statusMessage =
      user.employment_status === "draft"
        ? "Profil karyawan Anda belum diaktifkan."
