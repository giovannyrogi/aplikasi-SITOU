...
async function markQuarantinedFilesPurged(fileIds, organizationId) {
...
/** Mengarantina file replaceable dan menandai metadata terhapus dalam transaksi pemilik. */
export async function stageStoredFilesForDeletion(
  ...
  const quarantined = await quarantineStoredFiles(files);
  ...
  return { fileIds, quarantined, organizationId };
}

export async function restoreStagedStoredFiles(staged) {
  if (staged?.quarantined) await restoreQuarantinedFiles(staged.quarantined);
}

export async function purgeStagedStoredFiles(staged) {
  if (!staged) return;
  await purgeQuarantinedFiles(staged.quarantined || []);
  await markQuarantinedFilesPurged(staged.fileIds || [], staged.organizationId);
}

/** Menyiapkan seluruh file draft untuk dihapus dalam transaksi pemilik draft. */
export async function discardDraftFiles(
  ...
  const quarantined = await quarantineStoredFiles(files.rows);
...
/** Memulihkan byte yang sudah dikarantina bila transaksi pemilik draft gagal. */
export async function restoreDraftFiles(discarded) {
  if (discarded?.quarantined) await restoreQuarantinedFiles(discarded.quarantined);
}

/** Menghapus byte draft setelah commit dan menandai metadata telah dipurge. */
export async function purgeDraftFiles(discarded) {
  if (!discarded) return;
  await purgeQuarantinedFiles(discarded.quarantined || []);
  await markQuarantinedFilesPurged(discarded.fileIds || [], discarded.organizationId);
}
...
export async function storeEmployeeDraftFile({
  ...
  try {
    const stored = await withTransaction(async (client) => {
      ...
      quarantined = await quarantineStoredFiles(previous.rows);
      ...
      const inserted = await client.query(
        `INSERT INTO stored_files
         ...
         VALUES ($1,$2,'local_private',$3,$4,$5,$6,$7,$8,true,$9,'draft',$10)
    ...
    await purgeQuarantinedFiles(quarantined);
    await markQuarantinedFilesPurged(replacedIds, organizationId);
    return stored;
  } catch (error) {
    await restoreQuarantinedFiles(quarantined).catch(() => {});
    await discardPreparedEmployeeFile(prepared);
    throw error;
  }
}
...
export async function softDeleteDraftFile(fileId, draftId, organizationId, actor, requestId) {
  ...
  try {
    await withTransaction(async (client) => {
      ...
      quarantined = await quarantineStoredFiles(found.rows);
      await client.query(
        `UPDATE stored_files
         SET lifecycle_status='deleted',deleted_at=now(),deleted_by_user_id=$2,
             deletion_reason_code='removed_by_user'
         WHERE id=$1 AND organization_id=$3`,
        [fileId, actor.id, organizationId],
      );
      await writeAudit(client, {
        organizationId,
        actorUserId: actor.id,
    ...
    await purgeQuarantinedFiles(quarantined);
    await markQuarantinedFilesPurged([fileId], organizationId);
  } catch (error) {
    await restoreQuarantinedFiles(quarantined).catch(() => {});
    throw error;
  }
