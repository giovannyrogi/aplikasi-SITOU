  ...
  return withTransaction(async (client) => {
    ...
    const usedByActiveAction = await client.query(
      `SELECT 1 FROM disciplinary_actions
       WHERE organization_id=$1 AND document_file_id=$2 AND status='active'
    ...
    const usedByLeave = await client.query(
      "SELECT 1 FROM leave_request_attachments WHERE organization_id=$1 AND file_id=$2 LIMIT 1",
      [organizationId, fileId],
    );
    if (usedByLeave.rows[0])
      throw new ServiceError(
        "FILE_IN_USE",
        "Lampiran cuti atau izin yang sudah tercatat tidak dapat dihapus.",
        409,
      );
    ...
    await client.query(
      `UPDATE stored_files SET deleted_at=now(),deleted_by_user_id=$3,
        deletion_reason_code=COALESCE(deletion_reason_code,'removed_by_user')
       WHERE id=$1 AND organization_id=$2`,
      [fileId, organizationId, actor.id],
    );
    await writeAudit(client, {
      organizationId,
      actorUserId: actor.id,
