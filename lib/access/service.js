...
const accountSelect = `SELECT user_account.id::text,user_account.username,user_account.email,
  user_account.full_name,user_account.phone,user_account.is_active,user_account.last_login_at,
...
function normalizeAccount(row) {
  return { ...row, location_ids: (row.locations || []).map((location) => location.id) };
}
...
export async function listOrganizationAccounts({
  ...
  const where = `WHERE membership.organization_id=$1
    AND ($2='' OR user_account.username ILIKE $2 OR user_account.email ILIKE $2
      OR user_account.full_name ILIKE $2 OR COALESCE(employee.employee_no,'') ILIKE $2)
  ...
  const group = `GROUP BY user_account.id,membership.id,organization.name,role.code,role.name,
    employee.id,employee.employee_no,employee.full_name`;
  const params = [organizationId, `%${search}%`, status, roleCode, pageSize, offset];
  const [rows, count] = await Promise.all([
    pool.query(
      `${accountSelect} ${where} ${group} ORDER BY user_account.full_name,user_account.id LIMIT $5 OFFSET $6`,
      params,
    ),
    pool.query(
...
export async function createOrganizationAccount(input, actor, requestId) {
  try {
    ...
    if (error?.code === "23505")
      throw new ServiceError(
        "DUPLICATE_ACCOUNT",
        "Username, email, atau profil pegawai sudah digunakan.",
        409,
      );
    throw error;
  }
  ...
  try {
    return await withTransaction(async (client) => {
      ...
      const updated = await client.query(
        `UPDATE users SET username=$2,email=$3,full_name=$4,phone=$5,is_active=$6,updated_at=now()
