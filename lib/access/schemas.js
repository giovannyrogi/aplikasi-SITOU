...
const optionalId = z.union([requiredId("Profil pegawai tidak valid."), z.null()]).optional().default(null);
const optionalOrganizationId = z.union([requiredId("Organisasi tidak valid."), z.null()]).optional();
const password = z
  .string()
...
const accountBase = z
  .object({
    ...
    username: z.string().trim().min(3).max(80).regex(/^[A-Za-z0-9._-]+$/, "Username tidak valid."),
  ...
  .superRefine((value, context) => {
    if (value.roleCode === "employee" && !value.employeeId)
      context.addIssue({ code: "custom", path: ["employeeId"], message: "Profil pegawai wajib dipilih untuk akun Karyawan." });
    if (value.roleCode === "hrd" && value.locationScopeMode === "selected" && !value.locationIds.length)
      context.addIssue({ code: "custom", path: ["locationIds"], message: "Pilih minimal satu lokasi." });
  });
...
export const accountUpdateSchema = accountBase.safeExtend({ version: z.string().datetime({ offset: true }) });
