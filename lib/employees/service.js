...
export async function createEmployeeAssignment(
  ...
  return withTransaction(async (client) => {
    ...
    const employee = await client.query(
      "SELECT id FROM employees WHERE id=$1 AND organization_id=$2 AND deleted_at IS NULL FOR UPDATE",
  ...
  return withTransaction(async (client) => {
    ...
    await client.query(
      `UPDATE employee_assignments SET effective_until=GREATEST(effective_from,$3::date)
        WHERE organization_id=$1 AND employee_id=$2 AND effective_until IS NULL`,
    ...
    await writeAudit(client, {
      ...
      entityId: id,
      beforeData: employeeAuditSnapshot(before.rows[0]),
      afterData: { employmentStatus: input.status, terminationDate: input.terminationDate },
      requestId,
    });
  ...
  const [assignments, contracts] = await Promise.all([
    ...
    database.query(
      `SELECT contract.id::text,contract.contract_no,contract.start_date::text,
        ...
        contract.updated_at,contract_file.original_name AS document_name,
