...
import { incompleteEmployeeSql } from "@/lib/employees/completeness";
...
import {
  replaceEmployeeProfileSections,
  softDeleteRemovedProfileFiles,
...
const employeeSelect = `SELECT employee.id::text,employee.organization_id::text,
  ...
  (contact.employee_id IS NOT NULL) AS has_contact_data,
  ...
  (SELECT count(*)::int FROM disciplinary_actions action
    WHERE action.organization_id=employee.organization_id AND action.employee_id=employee.id
      AND (action.status='appealed' OR (action.status='active'
        AND (action.effective_until IS NULL OR action.effective_until>=current_date)))) AS active_sanction_count
...
/** Memastikan seluruh master penempatan aktif, satu organisasi, dan unit tersedia pada tanggal efektif. */
async function validateAssignmentReferences(client, organizationId, assignment, actor) {
  ...
  const result = await client.query(
    `SELECT location.id AS location_id,location.is_active AS location_active,
      location.operational_from<=$6::date
        AND (location.operational_until IS NULL OR location.operational_until>=$6::date)
        AS location_operational,
      unit.id AS unit_id,unit.is_active AS unit_active,position.id AS position_id,
    ...
    LEFT JOIN organization_units unit ON unit.organization_id=$1 AND unit.id=$3
    LEFT JOIN positions position
      ON position.organization_id=$1 AND position.id=$4 AND position.is_active=true
    LEFT JOIN employees supervisor
      ON supervisor.organization_id=$1 AND supervisor.id=$5 AND supervisor.deleted_at IS NULL
    WHERE location.organization_id=$1 AND location.id=$2`,
    [
      organizationId,
  ...
  if (!reference)
    throw new ServiceError("LOCATION_INVALID", "Lokasi tidak ditemukan pada organisasi ini.", 400, {
      locationId: "Lokasi tidak ditemukan pada organisasi ini.",
    });
  if (!reference.location_active)
    throw new ServiceError("LOCATION_INACTIVE", "Lokasi yang dipilih sedang nonaktif.", 400, {
      locationId: "Pilih lokasi yang masih aktif.",
    });
  if (!reference.location_operational)
    throw new ServiceError(
      "LOCATION_PERIOD_INVALID",
      "Lokasi belum atau tidak lagi beroperasi pada tanggal efektif yang dipilih.",
      400,
      { effectiveFrom: "Pilih tanggal efektif dalam periode operasional lokasi." },
    );
  if (!reference.unit_id)
    throw new ServiceError(
      "ORGANIZATION_UNIT_INVALID",
      "Divisi atau unit tidak ditemukan pada organisasi ini.",
      400,
      { organizationUnitId: "Divisi atau unit tidak ditemukan pada organisasi ini." },
    );
  if (!reference.unit_active)
    throw new ServiceError(
      "ORGANIZATION_UNIT_INACTIVE",
      "Divisi atau unit yang dipilih sedang nonaktif.",
      400,
      { organizationUnitId: "Pilih Divisi atau Unit yang masih aktif." },
    );
  if (assignment.positionId && !reference.position_id)
    throw new ServiceError("POSITION_INVALID", "Jabatan tidak aktif atau tidak ditemukan.", 400, {
      positionId: "Pilih jabatan aktif pada organisasi ini.",
    });
  if (assignment.supervisorEmployeeId && !reference.supervisor_id)
    throw new ServiceError(
      ...
      400,
      {
        supervisorEmployeeId: "Pilih atasan yang tersedia pada organisasi ini.",
      },
    );
  if (!reference.unit_available)
    throw new ServiceError(
      "UNIT_LOCATION_PERIOD_INVALID",
      "Divisi atau unit belum tersedia di lokasi tersebut pada tanggal efektif yang dipilih.",
      400,
      {
        organizationUnitId: "Divisi atau unit belum berlaku di lokasi pada tanggal efektif ini.",
        effectiveFrom: "Sesuaikan tanggal efektif dengan periode operasional Divisi atau Unit.",
      },
    );
}
...
export async function listEmployees(
  ...
  const withoutActiveContract = employmentTypeId === "without_active_contract";
  ...
  const params = [
    ...
    organizationUnitId,
    positionId,
    withoutActiveContract ? null : employmentTypeId,
    sanction,
    createdByUserId,
    scopedLocationIds,
    pageSize,
  ...
  const where = `WHERE employee.deleted_at IS NULL
    ...
    AND ($2='all' OR employee.employment_status=$2)
    AND employee.organization_id=$3
    ${completeness === "incomplete" ? `AND ${incompleteEmployeeSql("employee")}` : ""}
    ${completeness === "complete" ? `AND NOT ${incompleteEmployeeSql("employee")}` : ""}
    ...
    ${
      withoutActiveContract
        ? `AND employee.employment_status IN ('active','probation','suspended')
      AND NOT EXISTS (
        SELECT 1 FROM employment_contracts active_contract
        WHERE active_contract.organization_id=employee.organization_id
          AND active_contract.employee_id=employee.id
          AND active_contract.status='active'
          AND active_contract.start_date<=current_date
          AND (active_contract.end_date IS NULL OR active_contract.end_date>=current_date)
      )`
        : ""
    }
    AND ($8='all' OR ($8='with_sanction' AND EXISTS(
      ...
      AND action.employee_id=employee.id AND (action.status='appealed' OR
        (action.status='active' AND (action.effective_until IS NULL OR action.effective_until>=current_date)))))
      OR ($8='without_sanction' AND NOT EXISTS(
      SELECT 1 FROM disciplinary_actions action WHERE action.organization_id=employee.organization_id
      AND action.employee_id=employee.id AND (action.status='appealed' OR
        (action.status='active' AND (action.effective_until IS NULL OR action.effective_until>=current_date))))))
    AND ($9::bigint IS NULL OR EXISTS(
      SELECT 1 FROM audit_logs creator_audit
      WHERE creator_audit.organization_id=employee.organization_id
        AND creator_audit.entity_type='employee' AND creator_audit.entity_id=employee.id::text
        AND creator_audit.action='employee.create' AND creator_audit.actor_user_id=$9))
    AND ($10::bigint[] IS NULL OR current_assignment.location_id=ANY($10::bigint[]))`;
  const rows = await database.query(
    `${employeeSelect} ${where} ORDER BY employee.full_name LIMIT $11 OFFSET $12`,
    params,
  );
...
/** Memperbarui data inti di dalam transaksi pemilik agar file dapat ikut disimpan atomik. */
async function updateEmployeeInTransaction(client, id, input, actor, requestId) {
...
export async function updateEmployee(id, input, actor, requestId) {
  try {
    return await withTransaction(async (client) => {
      await updateEmployeeInTransaction(client, id, input, actor, requestId);
...
/** Menyimpan edit data, KTP, dan pas foto dalam satu transaksi yang dapat dipulihkan. */
export async function updateEmployeeWithProfileFiles(id, input, pendingUploads, actor, requestId) {
    ...
    for (const upload of pendingUploads) {
      preparedUploads.push({
        ...
        prepared: await prepareEmployeeFileUpload({
          file: upload.file,
          fileKind: upload.target === "profilePhoto" ? "pas_foto" : "ktp",
          employeeId: id,
          organizationId: input.organizationId,
        }),
      });
  ...
  try {
    const result = await withTransaction(async (client) => {
      const before = await updateEmployeeInTransaction(client, id, input, actor, requestId);
      const currentKtp = await client.query(
        `SELECT file.id::text,file.category,file.deleted_at,file.object_key
           FROM employee_documents document
           JOIN stored_files file ON file.organization_id=document.organization_id
             AND file.id=document.file_id
          WHERE document.organization_id=$1 AND document.employee_id=$2
            AND document.document_type='ktp' AND file.deleted_at IS NULL
          ORDER BY document.created_at DESC,document.id DESC
          FOR UPDATE OF file,document`,
        [input.organizationId, id],
      );
      const previousFiles = {
        profilePhoto: before.profile_photo_file_id ? String(before.profile_photo_file_id) : null,
        ktp: currentKtp.rows[0]?.id || null,
      };
      const previousKtpFileIds = currentKtp.rows.map((file) => file.id);
      ...
      const removedFileIds = [
        ...(input.fileChanges.profilePhoto !== "keep" && previousFiles.profilePhoto
          ? [previousFiles.profilePhoto]
          : []),
        ...(input.fileChanges.ktp !== "keep" ? previousKtpFileIds : []),
      ];
      const retainedFileIds = [
        input.fileChanges.profilePhoto === "keep"
          ? previousFiles.profilePhoto
          : replacementIds.profilePhoto,
        ...(input.fileChanges.ktp === "keep" ? previousKtpFileIds : [replacementIds.ktp]),
      ].filter(Boolean);
    ...
    await purgeStagedStoredFiles(quarantinedFiles).catch((error) => {
      console.error("[employee.edit-profile-files.purge]", { requestId, error: error.message });
    });
    return result;
  } catch (error) {
    if (!transactionCommitted) {
      await restoreStagedStoredFiles(quarantinedFiles);
      await Promise.all(
...
async function withPreparedLifecycleDocument(options, operation) {
  return commitPreparedEmployeeFile(options, operation);
}

async function recordOfficialDocumentVersion(
  client,
  { type, organizationId, recordId, fileId, actorId, reason = null },
) {
  const config =
    type === "contract"
      ? {
          table: "employment_contract_document_versions",
          owner: "employment_contract_id",
        }
      : {
          table: "employee_assignment_document_versions",
          owner: "employee_assignment_id",
        };
  await client.query(
    `UPDATE ${config.table} SET is_current=false
     WHERE organization_id=$1 AND ${config.owner}=$2 AND is_current`,
    [organizationId, recordId],
  );
  if (!fileId) return;
  await client.query(
    `INSERT INTO ${config.table}
      (organization_id,${config.owner},file_id,version_no,is_current,correction_reason,created_by_user_id)
     SELECT $1,$2,$3,COALESCE(MAX(version_no),0)+1,true,$4,$5
     FROM ${config.table}
     WHERE organization_id=$1 AND ${config.owner}=$2`,
    [organizationId, recordId, fileId, reason, actorId],
  );
}
...
export async function createEmployeeAssignment(
  ...
  return withPreparedLifecycleDocument(
    { file: documentFile, fileKind: "sk_penempatan", employeeId, organizationId, actor, requestId },
    async (client, uploadedFile) => {
      ...
      if (String(input.supervisorEmployeeId || "") === String(employeeId))
        throw new ServiceError(
          ...
          400,
          { supervisorEmployeeId: "Pilih pegawai lain sebagai atasan." },
        );
        ...
        if (current.rows[0]) {
          if (input.effectiveFrom <= String(current.rows[0].effective_from).slice(0, 10))
            throw new ServiceError(
              ...
              409,
              { effectiveFrom: "Pilih tanggal setelah awal penempatan utama saat ini." },
            );
          await client.query(
      ...
      await recordOfficialDocumentVersion(client, {
        type: "assignment",
        organizationId,
        recordId: id,
  ...
  return withPreparedLifecycleDocument(
    ...
    async (client, uploadedFile) => {
      ...
      if (current.effective_until)
        throw new ServiceError(
          "ASSIGNMENT_HISTORY_READ_ONLY",
          "Penempatan historis hanya dapat dilihat dan tidak dapat diubah.",
          409,
        );
      ...
      if (input.assignmentType === "primary") {
        ...
        if (overlap.rowCount)
          throw new ServiceError(
            ...
            409,
            { effectiveFrom: "Pilih periode yang tidak bertumpuk dengan penempatan utama lain." },
          );
      }
      ...
      if (String(current.document_file_id || "") !== String(input.documentFileId || ""))
        await recordOfficialDocumentVersion(client, {
          type: "assignment",
          organizationId,
          ...
          fileId: input.documentFileId,
          actorId: actor.id,
          reason: input.correctionReason || input.notes || null,
        });
...
export async function createEmployeeContract(
  ...
  return withPreparedLifecycleDocument(
    { file: documentFile, fileKind: "kontrak", employeeId, organizationId, actor, requestId },
    async (client, uploadedFile) => {
      if (uploadedFile) input = { ...input, documentFileId: uploadedFile.id };
      if (input.status === "active" && !input.contractNo)
        throw new ServiceError(
          "CONTRACT_NUMBER_REQUIRED",
          "Nomor kontrak wajib diisi untuk kontrak aktif.",
          400,
          { contractNo: "Nomor kontrak wajib diisi untuk kontrak aktif." },
        );
      if (input.status === "active" && !input.documentFileId)
        throw new ServiceError(
          "CONTRACT_DOCUMENT_REQUIRED",
          "Dokumen kontrak wajib disertakan untuk kontrak aktif.",
          400,
          { documentFileId: "Pilih PDF kontrak sebelum menyimpan." },
        );
      await ensureActorEmployeeAccess(actor, employeeId, organizationId, client);
      await ensureEmployeeLifecycleEditable(client, employeeId, organizationId);
...
export async function correctEmployeeContract(
  ...
  return withPreparedLifecycleDocument(
    ...
    async (client, uploadedFile) => {
      ...
      if (String(before.rows[0].document_file_id || "") !== String(input.documentFileId || ""))
        await recordOfficialDocumentVersion(client, {
          type: "contract",
          organizationId,
          recordId: contractId,
  ...
  const result = await pool.query(
    `SELECT id::text,employee_no,full_name FROM employees
      WHERE organization_id=$1 AND deleted_at IS NULL
        AND employment_status IN ('active','probation') AND ($2::bigint IS NULL OR id<>$2)
...
export async function getEmployeeReferenceOptions(organizationId, actor, database = pool) {
  const scopedLocationIds = await getActorLocationScope(actor, database);
  const [locations, units, positions, employmentTypes, employees, createdByUsers] =
    await Promise.all([
      database.query(
      ...
      // Hanya tampilkan akun yang benar-benar pernah mencatat pegawai dalam organisasi dan scope ini.
      database.query(
        `SELECT creator_audit.actor_user_id::text AS id,
        COALESCE(NULLIF(identity.display_name,''),account.username) AS name
       FROM audit_logs creator_audit
       JOIN employees employee
         ON employee.organization_id=creator_audit.organization_id
        AND employee.id::text=creator_audit.entity_id AND employee.deleted_at IS NULL
       JOIN users account ON account.id=creator_audit.actor_user_id
       LEFT JOIN v_user_identity identity ON identity.user_id=creator_audit.actor_user_id
       WHERE creator_audit.organization_id=$1 AND creator_audit.entity_type='employee'
         AND creator_audit.action='employee.create' AND creator_audit.actor_user_id IS NOT NULL
       ...
       GROUP BY creator_audit.actor_user_id,identity.display_name,account.username
  ...
  return {
    ...
    createdByUsers: createdByUsers.rows,
  };
}
