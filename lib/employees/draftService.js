...
/** Memastikan dokumen wajib tersedia dan mengikutsertakan foto/KTP opsional saat finalisasi. */
async function loadDraftDocuments(client, draft) {
  const files = await client.query(
    `SELECT id::text,object_key,mime_type,category FROM stored_files
     WHERE organization_id=$1 AND onboarding_draft_id=$2 AND deleted_at IS NULL
       AND category IN ('employee_photo','identity','contract','assignment_decree') FOR UPDATE`,
    [draft.organization_id, draft.id],
  );
  const byCategory = new Map(files.rows.map((file) => [file.category, file]));
  if (!byCategory.has("contract"))
    throw new ServiceError(
      "CONTRACT_DOCUMENT_REQUIRED",
      "Dokumen kontrak aktif wajib diunggah.",
      400,
      { "contract.documentFileId": "Dokumen kontrak wajib diunggah." },
    );
  if (!byCategory.has("assignment_decree"))
    throw new ServiceError(
      "ASSIGNMENT_DOCUMENT_REQUIRED",
      "Dokumen SK penempatan wajib diunggah.",
      400,
      { "assignment.documentFileId": "Dokumen SK wajib diunggah." },
    );
  ...
  try {
    employee = await withTransaction(async (client) => {
      ...
      const parsed = employeeCreateSchema.safeParse({
        ...
        profile: draft.payload.profile || {},
        contract: {
          ...draft.payload.contract,
          status: "active",
          documentFileId: documents.byCategory.get("contract").id,
        },
        assignment: {
          ...draft.payload.assignment,
          assignmentType: "primary",
          changeType: "initial",
          documentFileId: documents.byCategory.get("assignment_decree").id,
        },
      });
      if (!parsed.success) {
        const fieldErrors = {};
