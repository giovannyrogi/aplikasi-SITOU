...
import {
  discardDraftFiles,
  listEmployeeDraftFiles,
  promoteEmployeeDraftFiles,
  purgeDraftFiles,
  restoreDraftFiles,
...
/** Menutup satu draft dan membersihkan semua byte file melalui lifecycle service. */
async function transitionEmployeeDraft(
  draftId,
  organizationId,
  actor,
  requestId,
  { status, reasonCode, requireExpired = false },
) {
  let discardedFiles;
  try {
    await withTransaction(async (client) => {
      const changed = await client.query(
        `UPDATE employee_onboarding_drafts
         ...
         WHERE id=$1 AND organization_id=$2 AND created_by_user_id=$3
           AND status='active'
           AND ($5::boolean=false OR expires_at<=now())
         RETURNING id`,
        [draftId, organizationId, actor.id, status, requireExpired],
      );
      if (!changed.rows[0])
        throw new ServiceError("DRAFT_NOT_FOUND", "Draft pegawai tidak ditemukan.", 404);
      discardedFiles = await discardDraftFiles(client, {
        draftId,
        organizationId,
        actorId: actor.id,
        reasonCode,
      ...
      await writeAudit(client, {
        ...
        action: status === "expired" ? "employee_draft.expire" : "employee_draft.discard",
        entityType: "employee_onboarding_draft",
        entityId: draftId,
        afterData: { status, deletedFileCount: discardedFiles.fileIds.length },
        requestId,
      });
    });
  } catch (error) {
    await restoreDraftFiles(discardedFiles).catch(() => {});
    throw error;
  }

  try {
    await purgeDraftFiles(discardedFiles);
  } catch (error) {
    console.error("[employee-draft.file-purge]", {
...
/** Mengambil draft aktif actor setelah draft kedaluwarsa ditutup dan dibersihkan. */
export async function getActiveEmployeeDraft(organizationId, actor) {
  const expired = await pool.query(
    `SELECT id::text FROM employee_onboarding_drafts
  ...
  for (const row of expired.rows)
    await transitionEmployeeDraft(row.id, organizationId, actor, null, {
      status: "expired",
      reasonCode: "draft_expired",
      requireExpired: true,
    }).catch((error) => {
...
/** Buang draft memakai transisi dan cleanup file yang sama dengan expiry. */
export async function discardEmployeeDraft(draftId, organizationId, actor, requestId) {
  await transitionEmployeeDraft(draftId, organizationId, actor, requestId, {
    status: "discarded",
    reasonCode: "draft_discarded",
  });
}
  ...
  const files = await client.query(
    `SELECT id::text,object_key,mime_type,category FROM stored_files
     WHERE organization_id=$1 AND onboarding_draft_id=$2 AND lifecycle_status='draft'
       AND deleted_at IS NULL AND category IN ('employee_photo','identity','education','contract','assignment_decree') FOR UPDATE`,
