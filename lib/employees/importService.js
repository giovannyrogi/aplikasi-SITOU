...
import { commitPreparedEmployeeFile } from "@/lib/files/storage";
...
import { getLegacyDependentRelationshipMessage } from "@/lib/employees/dependentRelationships";
import {
...
const BOOLEAN_KEYS = new Set(["isVerified", "isPrimary", "isDependent", "isHighest"]);
  ...
  return {
    ...
    lists: {
      ...
      supervisors: employees.rows.filter(
        (row) => !row.deleted_at && ["active", "probation"].includes(row.employment_status),
      ),
    },
  };
}
...
function addInstructionSheet(workbook) {
  ...
  sheet.getCell("A3").value = `Versi template: ${EMPLOYEE_IMPORT_TEMPLATE_VERSION}`;
  sheet.getCell("A3").font = { bold: true, color: { argb: "FF667085" } };
  ...
  [
    ...
    "Isi sheet Kontrak dan Penempatan bila status pegawai active atau probation.",
...
async function parseEmployeeWorkbook(buffer) {
  ...
  if (!isCurrentEmployeeImportTemplate(workbook.subject))
    throw new ServiceError(
      "IMPORT_TEMPLATE_OUTDATED",
      EMPLOYEE_IMPORT_TEMPLATE_OUTDATED_MESSAGE,
      400,
    );
  ...
  for (const definition of EMPLOYEE_IMPORT_SHEETS) {
    ...
    sheet.getRow(1).eachCell((cell, columnNumber) => {
      const header = String(cell.value || "")
        .replace(/^\*\s*/, "")
        .trim();
      headers.set(normalizeImportHeader(definition.name, header), columnNumber);
    });
...
function validateRowShape(row) {
  ...
  for (const [key, header, required] of definition.columns) {
    ...
    const legacyRelationshipMessage =
      row.sheetName === "Keluarga" && key === "relationship"
        ? getLegacyDependentRelationshipMessage(row.normalized[key])
        : null;
    if (legacyRelationshipMessage) addError(row, legacyRelationshipMessage);
    else if (
      optionGroup &&
      row.normalized[key] &&
...
async function validateParsedRows(rows, references, scopedLocationIds) {
  ...
  for (const employeeNo of employeeMap.keys()) {
    ...
    if (["active", "probation"].includes(core?.employmentStatus)) {
...
export async function createEmployeeImport({ file, organizationId, actor, requestId }) {
  ...
  return commitPreparedEmployeeFile(
    {
      file,
      fileKind: "employee_import",
      employeeId: null,
      organizationId,
      actor,
      requestId,
    },
    async (client, sourceFile) => {
...
async function insertProfileRows(client, rows, organizationId, employeeId) {
  for (const row of rows) {
    ...
    if (row.sheet_name === "Keluarga")
      await client.query(
        "INSERT INTO employee_dependents(organization_id,employee_id,relationship,full_name,birth_date,national_id,is_dependent,notes) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)",
        [
          organizationId,
          employeeId,
          value.relationship,
...
export async function commitEmployeeImport(id, organizationId, actor, requestId) {
  ...
  const legacyFamilyRows = await pool.query(
    `SELECT 1
     FROM employee_import_rows
     WHERE organization_id=$1 AND batch_id=$2 AND sheet_name='Keluarga'
       AND normalized_data ?| ARRAY['phone','isEmergencyContact']
     LIMIT 1`,
    [organizationId, id],
  );
  if (legacyFamilyRows.rows[0])
    throw new ServiceError(
      "IMPORT_TEMPLATE_OUTDATED",
      EMPLOYEE_IMPORT_TEMPLATE_OUTDATED_MESSAGE,
      409,
