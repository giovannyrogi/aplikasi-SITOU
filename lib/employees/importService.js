...
function normalizeRow(definition, raw) {
  return Object.fromEntries(
    definition.columns.map(([key]) => {
      ...
      return [key, codeLike ? value?.toUpperCase() || null : value];
    }),
  );
}
...
async function loadImportReferences(database, organizationId) {
  const [types, locations, units, positions, employees, unitLocations] = await Promise.all([
    ...
    database.query(
      `SELECT id::text,upper(trim(employee_no)) employee_no,
...
function addInstructionSheet(workbook) {
  ...
  [
    ...
    "Gunakan Nomor Pegawai yang sama saat mengisi sheet lain milik pegawai tersebut.",
    "Isi sheet Kontrak dan Penempatan bila status pegawai active, probation, atau leave.",
    "Hapus seluruh baris contoh dengan Nomor Pegawai CONTOH-001 dari semua sheet.",
  ...
  [
    [
      "Nomor Pegawai",
      "Harus unik, konsisten di semua sheet, dan tidak boleh sudah terdaftar di SITOU.",
    ],
...
function addReferenceSheet(workbook, references) {
  ...
  const groups = [
    ["employmentTypes", "Jenis Kepegawaian", references.lists.types.map((row) => row.code)],
    ["locations", "Lokasi", references.lists.locations.map((row) => row.code)],
    ["units", "Divisi & Unit", references.lists.units.map((row) => row.code)],
    ["positions", "Jabatan", references.lists.positions.map((row) => row.code)],
  ];
...
function addTemplateValidations(sheet, definition, referenceRanges) {
  const rules = {
    gender: IMPORT_ENUMS.gender,
    employmentStatus: IMPORT_ENUMS.employmentStatus,
    identifierType: IMPORT_ENUMS.identifierType,
    relationship: IMPORT_ENUMS.relationship,
    isVerified: IMPORT_ENUMS.boolean,
    isPrimary: IMPORT_ENUMS.boolean,
    isDependent: IMPORT_ENUMS.boolean,
    isEmergencyContact: IMPORT_ENUMS.boolean,
    isHighest: IMPORT_ENUMS.boolean,
    status: definition.name === "Kontrak" ? IMPORT_ENUMS.contractStatus : null,
    assignmentType: IMPORT_ENUMS.assignmentType,
    changeType: IMPORT_ENUMS.changeType,
  ...
  for (const [key] of definition.columns) {
    ...
    const formula = Array.isArray(values) ? `"${values.join(",")}"` : values;
    sheet.dataValidations.add(`${column.letter}2:${column.letter}5001`, {
      type: "list",
      allowBlank: true,
      formulae: [formula],
      showErrorMessage: true,
      errorTitle: "Pilihan tidak valid",
      error: "Pilih nilai yang tersedia pada daftar.",
    });
  }
}
...
export async function createEmployeeImportTemplate(organizationId, database = pool) {
  ...
  for (const definition of EMPLOYEE_IMPORT_SHEETS) {
    ...
    sheet.addRow(getImportExample(definition.name));
...
function validateRowShape(row) {
  ...
  for (const [key, header, required] of definition.columns) {
    ...
    if (enums[key] && row.normalized[key] && !enums[key].includes(row.normalized[key]))
      addError(row, `${header} tidak termasuk pilihan yang didukung.`);
  }
  const statusEnums = { Kontrak: IMPORT_ENUMS.contractStatus };
  if (
    statusEnums[row.sheetName] &&
    row.normalized.status &&
    !statusEnums[row.sheetName].includes(row.normalized.status)
  )
    addError(row, "Status tidak termasuk pilihan yang didukung.");
  for (const [key, header] of definition.columns) {
...
async function validateParsedRows(rows, references, scopedLocationIds) {
  ...
  for (const row of employeeRows) {
    ...
    if (number && employeeMap.has(number)) {
      addError(row, "Nomor Pegawai duplikat di dalam workbook.");
      addError(employeeMap.get(number), "Nomor Pegawai duplikat di dalam workbook.");
    } else if (number) employeeMap.set(number, row);
    if (number && references.employees.has(number)) addError(row, "Nomor Pegawai sudah digunakan.");
  ...
  for (const row of rows) {
    ...
    if (row.sheetName !== "Pegawai" && employeeNo && !employeeMap.has(employeeNo))
      addError(row, "Nomor Pegawai tidak ditemukan pada sheet Pegawai.");
  }
  ...
  for (const row of rows.filter((item) => item.sheetName === "Penempatan")) {
    ...
    if (supervisorNo && !references.employees.has(supervisorNo) && !employeeMap.has(supervisorNo))
      addError(row, "Nomor Pegawai Atasan tidak ditemukan.");
  }
...
async function commitEmployeeGroup({
  ...
  return withTransaction(async (client) => {
    ...
    if (duplicate.rows[0])
      throw new ServiceError(
        "EMPLOYEE_DUPLICATE",
        "Nomor Pegawai atau NIK sudah digunakan pada organisasi ini.",
        409,
      );
    ...
    try {
      ...
      const message =
        error.code === "23505" || error.code === "EMPLOYEE_DUPLICATE"
          ? "Commit gagal: Nomor Pegawai atau NIK sudah digunakan."
...
export async function createEmployeeImportErrorReport(id, organizationId) {
  ...
  sheet.columns = [
    { header: "Nomor Pegawai", key: "employeeNo", width: 24 },
