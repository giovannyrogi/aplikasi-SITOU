import { z } from "zod";
import { requiredIndonesianMobileSchema } from "../validation/indonesianPhone.js";
import { optionalIndonesianNationalIdSchema } from "../validation/indonesianNationalId.js";
import { DEPENDENT_RELATIONSHIP_VALUES } from "./dependentRelationships.js";
  ...
  .superRefine((value, context) => {
    if (Object.hasOwn(value, "phone"))
      context.addIssue({
        ...
        path: ["phone"],
        message:
          "Nomor kontak keluarga sudah tidak digunakan. Tambahkan nomor pada bagian Kontak darurat.",
      });
    if (Object.hasOwn(value, "isEmergencyContact"))
      context.addIssue({
        code: "custom",
        path: ["isEmergencyContact"],
        message:
          "Penanda kontak darurat keluarga sudah tidak digunakan. Kelola kontak pada bagian Kontak darurat.",
      });
  })
  .transform((value) => {
    const result = { ...value };
    delete result.phone;
    delete result.isEmergencyContact;
    return result;
  });
...
export const employeeProfileSectionsSchema = z
  .object({
    ...
    bankAccounts: z
      ...
      .max(10)
      .default([]),
    dependents: z.array(dependentSchema).max(30).default([]),
    emergencyContacts: z
      .array(
        z.object({
          fullName: z.string().trim().min(1).max(200),
