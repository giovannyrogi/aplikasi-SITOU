...
/** Perubahan penempatan setelah onboarding tetap membutuhkan bukti SK resmi. */
export const assignmentSchema = createAssignmentSchema({ requireOfficialDocument: true });
...
export const employeeAssignmentCorrectionSchema = z
  .object({
    ...
    decreeNo: z.string().trim().min(1, "Nomor SK wajib diisi.").max(100),
    documentFileId: requiredId("Dokumen SK wajib tersedia."),
    notes: nullableText(2000),
    version: z.string().datetime({ offset: true }),
  })
  .superRefine((value, context) => {
