...
export const EDITABLE_EMPLOYMENT_STATUSES = ["draft", "active", "probation", "suspended"];
...
const createAssignmentSchema = ({ requireOfficialDocument }) =>
  z
    .object({
    ...
    .superRefine((value, context) => {
      if (requireOfficialDocument && !value.decreeNo)
        context.addIssue({
          code: "custom",
          path: ["decreeNo"],
          message: "Nomor dokumen penempatan wajib diisi.",
        });
      if (requireOfficialDocument && !value.documentFileId)
        context.addIssue({
          code: "custom",
          path: ["documentFileId"],
          message: "Dokumen penempatan wajib diunggah.",
        });
    });

/** Penempatan awal boleh disimpan sebelum nomor dan dokumen penempatan tersedia. */
export const initialAssignmentSchema = createAssignmentSchema({ requireOfficialDocument: false });

/** Penempatan baru dapat dicatat sebelum nomor dan PDF dokumen penempatan tersedia. */
export const assignmentSchema = createAssignmentSchema({ requireOfficialDocument: false });
...
/** Kontrak baru di luar onboarding tetap mewajibkan nomor dan dokumen resmi. */
...
const employeeProfileFileActionSchema = z.enum(["keep", "remove", "replace"]);

/** Payload edit multipart menjaga perubahan data dan file profil dalam satu penyimpanan. */
export const employeeUpdateMultipartSchema = employeeUpdateSchema
  .extend({
    fileChanges: z
      .object({
        profilePhoto: employeeProfileFileActionSchema.default("keep"),
        ktp: employeeProfileFileActionSchema.default("keep"),
      })
      .strict()
      .default({}),
    uploads: z
      .array(
        z.object({
          token: z.string().uuid(),
          target: z.enum(["profilePhoto", "ktp"]),
        }),
      )
      .max(2)
      .default([]),
  ...
  .superRefine((value, context) => {
    ...
    if (new Set(tokens).size !== tokens.length)
      context.addIssue({
        code: "custom",
        path: ["uploads"],
        message: "Setiap file wajib memiliki token yang berbeda.",
      });

    for (const target of ["profilePhoto", "ktp"]) {
      const targetUploads = value.uploads.filter((upload) => upload.target === target);
      const shouldReplace = value.fileChanges[target] === "replace";
      if ((shouldReplace && targetUploads.length !== 1) || (!shouldReplace && targetUploads.length))
        context.addIssue({
          code: "custom",
          path: ["fileChanges", target],
          message: shouldReplace
            ? "File pengganti belum lengkap. Pilih kembali file tersebut."
            : "File hanya boleh dikirim ketika pilihan penggantian aktif.",
        });
    }
  });
...
export const employeeAssignmentCorrectionSchema = z
  .object({
    organizationId: requiredId("Organisasi wajib dipilih."),
    locationId: requiredId("Lokasi wajib dipilih."),
    organizationUnitId: requiredId("Divisi atau unit wajib dipilih."),
    positionId: optionalId,
    supervisorEmployeeId: optionalId,
    assignmentType: z.enum(["primary", "acting", "temporary", "additional"]),
    changeType: z.enum([
      "initial",
      "rotation",
      "transfer",
    ...
    version: z.string().datetime({
      offset: true,
      error: "Versi data penempatan tidak valid. Muat ulang halaman lalu coba kembali.",
    }),
  })
  .superRefine((value, context) => {
    if (value.effectiveUntil && value.effectiveUntil < value.effectiveFrom)
      context.addIssue({
        code: "custom",
        path: ["effectiveUntil"],
        message: "Tanggal akhir penempatan tidak boleh sebelum tanggal efektif.",
      });
  });

export const employeeContractCreateSchema = initialContractSchema.extend({
  organizationId: requiredId("Organisasi wajib dipilih."),
});
...
export const employeeListFilterSchema = z.object({
  ...
  positionId: optionalId,
  employmentTypeId: z.union([z.literal("without_active_contract"), optionalId]),
  completeness: z.enum(["all", "incomplete", "complete"]).default("all"),
  createdByUserId: optionalId,
  employmentStatus: z
...
/** Satu parser filter dipakai daftar dan export agar cakupan hasil tidak berbeda. */
export function parseEmployeeListFilters(searchParams) {
  return employeeListFilterSchema.safeParse({
    organizationId: searchParams.get("organizationId") || null,
    locationId: searchParams.get("locationId") || null,
    organizationUnitId: searchParams.get("organizationUnitId") || null,
    positionId: searchParams.get("positionId") || null,
    employmentTypeId: searchParams.get("employmentTypeId") || null,
    completeness: searchParams.get("completeness") || "all",
    createdByUserId: searchParams.get("createdByUserId") || null,
    employmentStatus: searchParams.get("employmentStatus") || "all",
    sanction: searchParams.get("sanction") || "all",
  });
}
