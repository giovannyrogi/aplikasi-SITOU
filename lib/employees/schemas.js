import { z } from "zod";
import { employeeProfileSectionsSchema } from "./profileSchemas.js";
...
const optionalNationalId = z.preprocess(
  ...
  z
    .union([z.string().regex(/^\d{16}$/, "NIK harus terdiri dari tepat 16 digit."), z.null()])
    .optional()
    .default(null),
);
...
const employeeIdentitySchema = z.object({
  ...
  employeeNo: z
    .string()
    .trim()
    .min(1, "Nomor pegawai wajib diisi.")
    .max(60)
...
export const contractSchema = z
  ...
  .superRefine((value, context) => {
    ...
    if (value.status === "active" && !value.contractNo)
      context.addIssue({
        code: "custom",
        path: ["contractNo"],
        message: "Nomor kontrak wajib diisi untuk kontrak aktif.",
      });
    if (value.status === "active" && !value.documentFileId)
      context.addIssue({
        code: "custom",
        path: ["documentFileId"],
...
/** Payload checkpoint draft hanya menerima struktur wizard yang dikenal dan divalidasi penuh saat submit. */
export const employeeDraftSaveSchema = z.object({
  organizationId: requiredId("Organisasi wajib dipilih."),
  currentStep: z.coerce.number().int().min(0).max(2),
