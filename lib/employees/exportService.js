...
async function readEmployeeExportSnapshot(database, { filters, search, organizationId, actor }) {
  ...
  const sheetRowCounts = {
    ...
    "Kelengkapan Dokumen": employees.length,
    Kontak: employees.length,
    Rekening: outputCount(sections.bankAccounts),
    Keluarga: outputCount(sections.dependents),
    "Kontak Darurat": outputCount(sections.emergencyContacts),
    "Akun Sosial": outputCount(sections.socialAccounts),
    ...
    "Riwayat Kontrak": outputCount(sections.contracts),
    "Riwayat Penempatan": outputCount(sections.assignments),
  };
