...
import { DEPENDENT_RELATIONSHIP_OPTIONS } from "./dependentRelationships.js";

/** Versi berubah ketika struktur workbook tidak lagi kompatibel dengan template lama. */
export const EMPLOYEE_IMPORT_TEMPLATE_VERSION = 2;
export const EMPLOYEE_IMPORT_TEMPLATE_SUBJECT = `SITOU_EMPLOYEE_IMPORT_V${EMPLOYEE_IMPORT_TEMPLATE_VERSION}`;
export const EMPLOYEE_IMPORT_TEMPLATE_OUTDATED_MESSAGE =
  "Template import sudah tidak berlaku. Unduh template terbaru dan pindahkan Nomor Kontak serta Kontak Darurat dari sheet Keluarga ke sheet Kontak_Darurat.";

export function isCurrentEmployeeImportTemplate(subject) {
  return subject === EMPLOYEE_IMPORT_TEMPLATE_SUBJECT;
}
...
/** Header lama tetap diterima agar workbook yang sudah diunduh tidak rusak setelah perubahan istilah. */
export const EMPLOYEE_IMPORT_HEADER_ALIASES = Object.freeze({
  Pegawai: Object.freeze({
    "Tanggal Bergabung": "Tanggal Bergabung di Organisasi",
  }),
  Penempatan: Object.freeze({
    "Nomor SK": "Nomor Dokumen Penempatan",
  }),
});
export function normalizeImportHeader(sheetName, header) {
  return EMPLOYEE_IMPORT_HEADER_ALIASES[sheetName]?.[header] || header;
}
...
export const EMPLOYEE_IMPORT_SHEET_GUIDANCE = Object.freeze([
  ...
  },
  {
    ...
    requirement: "optional",
    requirementLabel: "Opsional",
    purpose: "Menyimpan istri, suami, anak, orang tua, atau anggota keluarga lain.",
    whenToFill: "Isi satu baris untuk setiap anggota keluarga yang perlu dicatat.",
    importantRule:
      "Gunakan NIP yang sama dengan sheet Pegawai. Nomor telepon hanya diisi pada sheet Kontak_Darurat.",
  },
  {
    name: "Kontak_Darurat",
    requirement: "optional",
  ...
  },
  {
    ...
    whenToFill: "Wajib untuk pegawai berstatus aktif atau masa percobaan (active atau probation).",
    importantRule:
...
export function getImportExample(sheetName) {
  ...
  const examples = {
    ...
    Keluarga: {
      ...common,
      relationship: getImportOptionLabel("dependentRelationship", "wife"),
