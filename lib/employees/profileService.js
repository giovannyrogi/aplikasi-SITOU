...
export async function getEmployeeProfileSections(
  ...
  const [
    ...
    database.query(
      "SELECT id::text,relationship,full_name,birth_date::text,national_id,is_dependent,notes FROM employee_dependents WHERE organization_id=$1 AND employee_id=$2 ORDER BY full_name",
      [organizationId, employeeId],
    ),
    database.query(
...
export async function replaceEmployeeProfileSections(
  client,
  employeeId,
  organizationId,
  profile,
  ...
  if (fileIds.length) {
    const validFiles = await client.query(
      `SELECT id::text,category FROM stored_files
       WHERE organization_id=$1 AND deleted_at IS NULL AND id=ANY($3::bigint[])
         AND (employee_id=$2 OR ($4::bigint IS NOT NULL AND onboarding_draft_id=$4))`,
      [organizationId, employeeId, fileIds, onboardingDraftId],
    );
  ...
  // NIK inti tetap dipertahankan ketika section administratif tidak membawa baris KTP.
  if (ktp)
...
export async function softDeleteRemovedProfileFiles(
  client,
  employeeId,
  organizationId,
  removedFileIds,
  ...
  const stagedFiles = await stageStoredFilesForDeletion(client, files.rows, {
    organizationId,
    actorId: actor.id,
    reasonCode: "profile_removed",
  });
  ...
  await client.query(
    `UPDATE stored_files SET lifecycle_status='deleted',deleted_at=now(),deleted_by_user_id=$4,
      deletion_reason_code=COALESCE(deletion_reason_code,'profile_removed')
      WHERE organization_id=$1 AND employee_id=$2 AND deleted_at IS NULL
        AND id=ANY($3::bigint[])`,
    [organizationId, employeeId, requested, actor.id],
  );
...
export async function updateEmployeeProfileSections(
  ...
  try {
    ...
    await purgeStagedStoredFiles(quarantinedFiles).catch((error) => {
      console.error("[employee.profile-files.purge]", { requestId, error: error.message });
    });
    return result;
  } catch (error) {
