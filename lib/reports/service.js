...
export async function getReportContext(actor, requestedOrganizationId) {
  ...
  const result = await pool.query(
    `SELECT o.id::text,o.name,o.timezone,p.retirement_age,p.version AS retirement_policy_version
     FROM organizations o LEFT JOIN organization_retirement_policies p ON p.organization_id=o.id
...
export async function readReport(
  ...
  if (kind === "retirements" && !context.organization.retirement_age)
    throw new ServiceError(
      "RETIREMENT_POLICY_NOT_CONFIGURED",
      "Kebijakan usia pensiun belum diatur. Hubungi HRD atau buka Pengaturan Organisasi → Kebijakan Pensiun.",
      409,
    );
  ...
  const signature = createHash("sha256")
    .update(
      JSON.stringify([
        ...
        context.scope,
        context.today,
        context.organization.retirement_policy_version,
      ]),
    )
    .digest("hex");
  let cursor = null;
  ...
  const query = buildReportQuery(
    ...
    limit,
    cursor,
    context.organization.retirement_age,
  );
  const {
    rows: [result],
    ...
    definitions.map(async ([kind, input, key, label, tone]) => {
      if (kind === "retirements" && !context.organization.retirement_age)
        return {
          ...
          asOf: context.today,
          retirementAge: null,
          description: "Kebijakan usia pensiun belum diatur.",
          href: `/organization-settings/retirement?organizationId=${organizationId}`,
        };
      ...
      return {
        ...
        ...(kind === "retirements"
          ? {
              rows: report.rows,
              asOf: report.asOf,
              retirementAge: context.organization.retirement_age,
            }
          : {}),
      };
    }),
