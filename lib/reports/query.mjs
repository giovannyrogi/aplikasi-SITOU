...
export function buildReportQuery(
  ...
  if (retirement && (!Number.isInteger(retirementAge) || retirementAge < 18 || retirementAge > 100))
    throw new Error("Kebijakan usia pensiun organisasi belum valid.");
  const ageParameter = retirement ? bind(retirementAge) : null;
  const conditions = [];
  ...
  const text = `WITH base AS (
    SELECT e.id AS employee_id,e.organization_id,photo.id AS profile_photo_file_id,e.employee_no,e.full_name,e.birth_date,e.joined_date,e.employment_status,
      ...
      ${
        ...
        THEN (e.birth_date+make_interval(years => ${ageParameter}::int))::date END AS due_date,
