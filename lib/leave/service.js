...
import { commitPreparedEmployeeFiles } from "@/lib/files/storage";
...
export async function createLeaveRequest(input, actor, requestId, attachmentFiles = []) {
  return commitPreparedEmployeeFiles(
    {
      uploads: attachmentFiles.map((file) => ({ file, fileKind: "lampiran_cuti" })),
      employeeId: input.employeeId,
      organizationId: input.organizationId,
      actor,
      requestId,
    },
    async (client, uploadedFiles) => {
      input = {
        ...input,
        attachmentFileIds: [
    ...
    if (input.attachmentFileIds.length) {
      const files = await client.query(
        "SELECT id FROM stored_files WHERE organization_id=$1 AND employee_id=$2 AND id=ANY($3::bigint[]) AND category='leave_attachment' AND lifecycle_status='active' AND deleted_at IS NULL",
