...
import { positionCreateSchema } from "@/lib/master-data/schemas";
import { createPosition, listPositions } from "@/lib/master-data/organizationStructureService";

/** Menampilkan jabatan lintas organisasi untuk Superadmin atau organisasi session HRD. */
export async function GET(request) {
  ...
  try {
    ...
    const result = await listPositions({ ...query, organizationId: scope.organizationId });
    return successResponse(result.data, {
      pagination: { page: query.page, pageSize: query.pageSize, total: result.total },
    });
  } catch (error) {
    return handleRouteError("positions.list", error, requestId);
  }
}

/** Membuat jabatan pada organisasi yang telah diverifikasi server. */
export async function POST(request) {
  ...
  const parsed = await readJson(request, positionCreateSchema, requestId);
  ...
  try {
    ...
    return handleRouteError("positions.create", error, requestId);
  }
}
