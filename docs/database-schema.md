...
| `employee_identifiers`        | KK, BPJS, NPWP, dan identitas administratif lain.                       | `employee_id`, `identifier_type`, `identifier_label`, `identifier_value`, `issued_at`, `expires_at`, `is_verified`, `document_file_id`. KTP tetap memakai `employees.national_id`; UI tidak menawarkan paspor.                                |
...
| `employment_contracts`       | Histori kontrak pegawai. Kontrak baru menutup kontrak aktif sehari sebelum tanggal mulai baru dan menandainya `renewed`; salah input dapat dikoreksi dengan version check atau dibatalkan secara logis, tetapi record tidak dihapus. | `employee_id`, `employment_type_id`, `contract_no`, `start_date`, `end_date`, `status`, `document_file_id`, `notes`, `updated_at`, `cancelled_at`, `cancellation_reason`, `cancelled_by_user_id`.      |
...
`employee_onboarding_drafts` hanya menyimpan satu draft aktif per actor dan organisasi. Penyimpanan checkpoint menggunakan `version` untuk optimistic concurrency. Finalisasi mengunci draft, membuat pegawai, kontak, kontrak, dan penempatan awal secara atomik, lalu menghubungkan file staging kepada pegawai. Pas foto kategori `employee_photo` bersifat opsional dan ditautkan melalui `employees.profile_photo_file_id`. KTP kategori `identity` juga opsional dan dicatat sebagai `employee_documents.document_type='ktp'`. Kontrak aktif wajib memiliki nomor dan `document_file_id` kategori `contract`; penempatan aktif wajib memiliki nomor SK dan `document_file_id` kategori `assignment_decree`.

Koreksi atau pembatalan kontrak menggunakan `PATCH/DELETE /api/employees/:employeeId/contracts/:contractId`. `DELETE` bersifat pembatalan logis, wajib menerima alasan dan versi record, serta tidak menghapus dokumen maupun histori audit. Dokumen kontrak dan SK dikembalikan pada histori masing-masing dan masing-masing record hanya menyediakan satu aksi dokumen agar tidak ada kontrol berbeda dengan perilaku sama.
